...
TEST_CASE("Not Inited Throw", "[sort][throw]") {
  ...
  REQUIRE_THROWS_WITH(bench.sorted_arrays(), "Uninitialized arrays, use operator() to fill it first");
  REQUIRE_THROWS_WITH(bench.sorted_arrays(), "Uninitialized arrays, use operator() to fill it first");
}
...
TEST_CASE("Not Kept Arrays Throw", "[sort][throw]") {
  ...
  REQUIRE_THROWS_AS(bench.notsorted_arrays(), std::logic_error);
  REQUIRE_THROWS_WITH(bench.notsorted_arrays(), "Call keep_before(), and than operator() to get this arrays");
  ...
  REQUIRE_THROWS_AS(bench.sorted_arrays(), std::logic_error);
  REQUIRE_THROWS_WITH(bench.sorted_arrays(), "Call keep_after(), and than operator() to get this arrays");
}